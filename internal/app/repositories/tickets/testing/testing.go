	...
	for rowIndex, objectID := range objectIDs {
		if objectID.(primitive.ObjectID).Hex() == ticketID {
			...
			for key, value := range r.Collection.Columns {
				...
				err := reflections.SetField(ticketFound, key, cellValue)
				if err != nil {
					return nil, err
				}
			}
		...
		if objectID.(primitive.ObjectID).Hex() == ticketID {
			...
			if updateMap, ok := update.(map[string]interface{}); ok {
				for key, value := range updateMap {
					err := reflections.SetField(ticketFound, key, value)
					if err != nil {
						return err
					}
				}
			...
			for key := range r.Collection.Columns {
				cellValue, err := reflections.GetField(ticketFound, key)
				if err != nil {
					return err
				}
			...
			for rowIndex, cell := range r.Collection.Columns[key] {
				if cell == value {
					...
					for fieldName, column := range r.Collection.Columns {
						...
						err := reflections.SetField(&newTicket, fieldName, value)
						if err != nil {
							return nil, err
						}
					}
					...
					if ticketModel, ok := cell.(models.Ticket); ok {
						...
						if updateMap, ok := update.(map[string]interface{}); ok {
							...
							for key, value := range updateMap {
								...
								err := reflections.SetField(&ticketModel, key, value)
								if err != nil {
									return nil, err
								}
