	...
	for rowIndex, objectID := range objectIDs {
		if objectID.(primitive.ObjectID).Hex() == sessionID {
			...
			for key, value := range r.Collection.Columns {
				...
				err := reflections.SetField(sessionFound, key, cellValue)
				if err != nil {
					return nil, err
				}
			}
		...
		if objectID.(primitive.ObjectID).Hex() == sessionID {
			...
			if updateMap, ok := update.(map[string]interface{}); ok {
				for key, value := range updateMap {
					err := reflections.SetField(sessionFound, key, value)
					if err != nil {
						return err
					}
				}
			...
			for key := range r.Collection.Columns {
				cellValue, err := reflections.GetField(sessionFound, key)
				if err != nil {
					return err
				}
			...
			for rowIndex, cell := range r.Collection.Columns[key] {
				if cell == value {
					...
					for fieldName, column := range r.Collection.Columns {
						...
						err := reflections.SetField(&newSession, fieldName, value)
						if err != nil {
							return nil, err
						}
					}
					modelsFound = append(modelsFound, newSession)
				}
			}
		}
	} else {
					...
					if sessionModel, ok := cell.(models.Session); ok {
						...
						if updateMap, ok := update.(map[string]interface{}); ok {
							...
							for key, value := range updateMap {
								...
								err := reflections.SetField(&sessionModel, key, value)
								if err != nil {
									return nil, err
								}
